...
counter = 0
cur_dir = 0
num_of_imgs_in_single_loop = 1
latents_total = []
for i in tqdm(range(NUMBER_OF_IMAGES // num_of_imgs_in_single_loop)):
    with torch.no_grad():
        sample_z = torch.randn(num_of_imgs_in_single_loop,512,device = 'cuda')
        sample,latents = generator(
            [sample_z],input_is_latent = False,return_latents = True,truncation = 0.5,
