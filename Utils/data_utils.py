...
def plot_single_w_image(w,generator):
    """
    Plot a single image from the generator"""
    w = w.unsqueeze(0).to(configs.device)
...
def get_data_by_idx(idx,root_dir,postfix):
    if torch.is_tensor(idx):
        idx = idx.tolist()
    dir_idx = idx // configs.num_images_per_dir
...
class Image_W_Dataset(Dataset):
    ...
    def __getitem__(self,idx):
        w = get_data_by_idx(idx,self.w_dir,'.npy')
        image = get_data_by_idx(idx,self.image_dir,'.png')
        return w,image

def cycles_images_to_create_diff_order(images):
    batch_size = len(images)
    different_images = torch.empty_like(images,device = configs.device)
