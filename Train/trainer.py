...
from Losses.adverloss import calc_Dw_loss, R1_regularization
import torch
from Losses.nonadverloss import landmark_loss,rec_loss,l2_loss
...
class Trainer:
    ...
    def __init__(self,config,
        ...
        adverserial_mapper_optimizer: torch.optim.Optimizer,
        non_adverserial_mapper_optimizer: torch.optim.Optimizer,
        discriminator,
        generator,
        id_encoder,
        attr_encoder,
        ...
        self.adverserial_mapper_optimizer = adverserial_mapper_optimizer
        self.non_adverserial_mapper_optimizer = non_adverserial_mapper_optimizer
        self.lpips_loss = lpips.LPIPS(net='alex').to(config.device).eval()


    def train_discriminator(self,real_w,generated_w):
        # pass
        self.discriminator.optimizer.zero_grad()
        real_w.requires_grad()
        ...
        r1_error = R1_regularization(self.config['R1Param'], prediction_real, real_w)
        r1_error.backward()
    ...
    def adversarial_train_step(self,real_w,fake_data):
        ...
        return error_real, error_fake, torch.mean(prediction_real), torch.mean(prediction_fake), g_error, torch.mean(g_pred)
    ...
    def non_adversarial_train_step(self, id_images, attr_images, fake_data, real_landmarks, use_rec_extra_term):
        ...
        total_loss = torch.tensor(0, dtype=torch.float, device=configs.device)
        ...
        if self.config['use_id']:
            ...
            wandb.log({'id_loss_val': id_loss_val.detach().cpu()}, step=configs.step)
        ...
        if self.config['use_landmark']:
            ...
            wandb.log({'landmark_loss_val': landmark_loss_val.detach().cpu()}, step=configs.step)
        ...
        if use_rec_extra_term and self.config['use_reconstruction']:
            ...
            wandb.log({'rec_loss_val': rec_loss_val.detach().cpu()}, step=configs.step)
        ...
        if use_rec_extra_term and self.config['use_l2'] > 0:
            ...
            wandb.log({'l2_loss_val': l2_loss_val.detach().cpu()}, step=configs.step)
        ...
        if use_rec_extra_term and (not self.config['use_adverserial']):
            ...
            wandb.log({'vgg_loss_val': vgg_loss_val.detach().cpu()}, step=configs.step)
